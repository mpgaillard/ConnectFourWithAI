...
class ComputerPlayer < Player
	...
	def minimax(board, move, depth, maximizing_player)
		...
		if board.next_available_row.max < 0
			...
			generate_possible_moves(board, move, depth, maximizing_player)
		end	
	end

	def generate_possible_moves(board, move, depth, maximizing_player)
		...
		(0...board.cols).each do |i|
			if board.valid_column?(i.to_s)
				...
				child = minimax(board, i, depth-1, !maximizing_player)
	...
	def choose_column(board)
		generate_possible_moves(board, -1, 6, true).move.to_s
		#get_input
	end
